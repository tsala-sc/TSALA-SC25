...
selected_columns = ['progName', 'jobID', 'startTime', 'numProc', 'numOST', 'stripeSize', 'totalFile', 'totalIOReq',
                   ...
                   'runTime', 'totalReadReq', 'totalWriteReq', 'totalOpenReq','totalSeekReq', 'totalStatReq']
...
# extract inputs
def extract_X_y(df):
    X = df.drop(['runTime','progName', 'jobID', 'progName_encoded'], axis=1).values
    y = df['runTime'].values
    return X, y
...
best_model_path = os.path.join(model_dir, f"tsala_system_runtime_withoutEmb.h5")
