...
from build_model_withoutEmb import build_model  
...
selected_columns = ['progName', 'jobID', 'startTime', 'numProc', 'numOST', 'stripeSize', 'totalFile', 'totalIOReq',
                   ...
                   'writeRateTotal', 'totalReadReq', 'totalWriteReq', 'totalOpenReq','totalSeekReq', 'totalStatReq']
...
data = data[data['writeRateTotal'] != 0]

# encode 'progName' to int (won't be used in model)
...
# extract inputs
def extract_X_y(df):
    X = df.drop(['writeRateTotal','progName', 'jobID', 'progName_encoded'], axis=1).values
    y = df['writeRateTotal'].values
    return X, y

X_train, y_train = extract_X_y(train_data)
X_valid, y_valid = extract_X_y(valid_data)
...
for sequence_length in sequence_lengths:
    X_train_seq, _, y_train_seq = create_sequences(X_train, None, y_train, sequence_length)
    X_valid_seq, _, y_valid_seq = create_sequences(X_valid, None, y_valid, sequence_length)
    X_test_seq, _, y_test_seq = create_sequences(X_test, None, y_test, sequence_length)

    model = build_model(sequence_length, n_features)  # no progName_size or embedding_dim
    ...
    predicted_data = model.predict(X_test_seq)
...
best_model_path = os.path.join(model_dir, f"tsala_system_write_withoutEmb.h5")
